...
* <<Check validity of a changePasswordId>>
...
[.api-authentication]
link:/docs/v1/tech/apis/authentication#no-authentication-required[icon:unlock-alt[role=green, title="No authentication required"]] Determines if a user's `changePasswordId` is valid.

[.endpoint]
.URI
--
[method]#GET# [uri]#/api/user/change-password/`\{changePasswordId\}`#
--
...
|404
|The changePasswordId is expired or incorrect.
