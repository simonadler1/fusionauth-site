...
Tenants also allow you to segment and limit trust of configured Identity Providers. If an Identity Provider in the Pied Piper tenant is malicious and creating accounts with hooli.com email addresses, there's no chance of gaining access to a user account in the Hooli tenant. Users within a tenant are entirely distinct from users in another tenant. Learn more about link:/docs/v1/tech/identity-providers/#account-security[account security and Identity Providers].
