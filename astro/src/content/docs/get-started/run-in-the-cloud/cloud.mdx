...
import AccountSectionsOverview from 'src/content/docs/get-started/run-in-the-cloud/_account-sections-overview.mdx';
import CloudUserNote from 'src/content/docs/get-started/run-in-the-cloud/_cloud-user-note.mdx';
import SettingUpPortalAccount from 'src/content/docs/get-started/run-in-the-cloud/_setting-up-portal-account.mdx';
...
You can read more about the other portal areas in the [Account Portal](/docs/v1/tech/admin-guide/account-portal) documentation.
...
Learn more about the [other portal areas here](/docs/v1/tech/admin-guide/account-portal).
...
At that point the [Setup Wizard](/docs/v1/tech/tutorials/setup-wizard) will begin.
...
If new to FusionAuth, you might want to work through the [5 minute guide](/docs/v1/tech/getting-started/5-minute-fastpath#5-create-an-application-and-configure-the-oauth-settings), starting at step 5, and updating the FusionAuth instance URL to point to your deployment.
...
This guide to [Common configuration](/docs/v1/tech/installation-guide/common-configuration) is helpful at this point.
...
* Navigate to the <strong>Deployments</strong> tab. Manage the deployment, then choose the "Upgrade" option. Select `1.x` from the [field]#Version# dropdown.
...
* After confirmation, the deployment will be in an "Upgrading" state until finished. Monitor the upgrade by viewing the <strong>Deployments</strong> tab. To programmatically monitor the upgrade, call the [System Status API](/docs/v1/tech/apis/system#retrieve-system-status).
...
<img src="/img/docs/get-started/run-in-the-cloud/support-tab.png" alt="The support tab" width="1200" role="bottom-cropped" />
...
FusionAuth Cloud has the same [limitations](/docs/v1/tech/reference/limitations) as self hosted FusionAuth. Since it is a managed service, there are additional limitations as well:
...
* You cannot change any of the [FusionAuth configuration options](/docs/v1/tech/reference/configuration).
...
* You cannot run a [Kickstart file](/docs/v1/tech/installation-guide/kickstart) on a FusionAuth Cloud deployment. (yet! 😎)
